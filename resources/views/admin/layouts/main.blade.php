...
<head>
    ...
    <title>@yield('title', 'Quản lý lịch khám')</title>
    <!-- Tell the browser to be responsive to screen width -->
    <meta name="csrf-token" content="{{ csrf_token() }}">
    <link rel="shortcut icon" type="image/png" href="{{ asset('page/img/images(2).png') }}" id="favicon" />
    ...
    <script>
        var baseURL = "{!! url('/')!!}"
        var loadLocation = "{{ route('ajax.post.load.location') }}"
        var loadListTimes = "{{ route('load.list.times') }}"
        var loadSpecialty = "{{ route('ajax.load.specialty') }}"
    </script>
    @yield('style-css')
